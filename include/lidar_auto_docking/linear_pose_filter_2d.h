...
class LinearPoseFilter2D {
 public:
   ...
   * state(s). new_poses[0] is the oldest sample and new_poses[new_poses.size()
   * - 1] is the newest sample. If the number of poses is greater than the
   * length of the filter memory, only the newest poses will be used. If the
   * number of poses is less than the length of the filter memory, the poses
   * will be copied to the most recent history and the original samples will
   * retain their original location in the buffer. Example:
   *                     // New poses.
   *                     np = [1, 2, 3]
   *                     // Original poses.
   *                     p  = [9, 8, 7, 6, 5]
 ...
 private:
   ...
   * reference yaw and returns it. Example: pose = -178 degrees, reference_yaw =
   * 177 degrees, return = 182 degrees. pose =  175 degrees, reference_yaw =
   * -170 degrees, return = -185 degrees.
  ...
  std::deque<geometry_msgs::msg::Pose> in_;  /// Vector of previous input poses.
  std::deque<float>
